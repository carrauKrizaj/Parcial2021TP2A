...
function updateRepo(newRepo,arrBeer){
    return arrBeer
            .map(beer => ({
              ...beer,
              label: newRepo+beer.label.split('/')[5]+'/'+beer.name.replace(' ',''),
              type: beer.type
            }))
}
...
// TESTS
const nuevoRepo = 'https://tecnoshare.sharepoint.com/sites/beer/';
console.log(updateRepo(nuevoRepo,beers));
